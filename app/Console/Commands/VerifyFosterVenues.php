...
class VerifyFosterVenues extends Command
{
    ...
    public function handle()
    {
        ...
        // Fetch pending venues from the database, capped at 50 per run for safety & budget control
    ...
    private function analyzeWithAI(string $name, string $locationContext, string $snippets): ?array
    {
        ...
        $prompt = "你是一位台灣流浪動物中途商家的「極度嚴格」審核專家。\n"
            ...
            . "3. 「僅捐款/義賣」：僅將部分盈餘捐給動保團體，或僅在店內置放發票箱、販售公益年曆，但店內並無實際安置中途浪浪。\n"
            . "4. 「名字誤導」：例如「小獸書屋」雖有「獸」字，但本質是獨立書店，除非有強烈證據顯示它「長期在店內安置流浪貓狗供人認養」，否則一律排除。\n"
